...
import { Activity, AlertTriangle, TrendingUp, Clock, Map, Target } from 'lucide-react'
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-yellow-700">
                <strong>Modo demostración:</strong> Usando datos de prueba. Las coordenadas pueden no coincidir con la configuración actual.
                {sensorData.length > 0 && (
                  <span className="block mt-1">
                    E4 posición Y: {sensorData.find(s => s.micro_id === 'E4')?.latitude || 'N/A'}m (configuración: 12m)
                  </span>
                )}
              </p>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="card">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className={`px-2 py-1 text-xs font-medium rounded-lg ${getNoiseLevelColor(stats.avgNoise)}`}>
              {getNoiseLevelLabel(stats.avgNoise)}
            </span>
          </div>
          <div className="text-3xl font-bold mb-1">{stats.avgNoise.toFixed(1)} dB</div>
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className={`px-2 py-1 text-xs font-medium rounded-lg ${getNoiseLevelColor(stats.maxNoise)}`}>
              {getNoiseLevelLabel(stats.maxNoise)}
            </span>
          </div>
          <div className="text-3xl font-bold mb-1">{stats.maxNoise.toFixed(1)} dB</div>
          <div className="text-sm text-primary-600">
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className={`px-2 py-1 text-xs font-medium rounded-lg ${
              connected ? 'text-green-600 bg-green-50 border-green-200' : 'text-red-600 bg-red-50 border-red-200'
            }`}>
              {connected ? 'En vivo' : 'Desconectado'}
            </span>
          </div>
          <div className="text-3xl font-bold mb-1">{formatTime(lastUpdate)}</div>
          <div className="text-sm text-primary-600">
       ...
       <div className="card">
         <div className="flex items-center justify-between">
           <h3 className="font-semibold text-lg">Controles de Visualización</h3>
           <div className="flex items-center space-x-4">
             <label className="flex items-center space-x-3 cursor-pointer">
               <div className="relative">
                 ...
                 <div className={`w-10 h-6 rounded-full transition-colors ${
                   showHeatmap ? 'bg-accent-500' : 'bg-primary-300'
                 }`}>
                   <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${
                     showHeatmap ? 'left-5' : 'left-1'
                   }`} />
                 </div>
               </div>
               <div className="flex items-center space-x-2">
             ...
             <label className="flex items-center space-x-3 cursor-pointer">
               <div className="relative">
                 ...
                 />
                 <div className={`w-10 h-6 rounded-full transition-colors ${
                   showEpicenter ? 'bg-accent-500' : 'bg-primary-300'
                 }`}>
                   <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${
                     showEpicenter ? 'left-5' : 'left-1'
                   }`} />
                 </div>
               </div>
               <div className="flex items-center space-x-2">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-primary-200">
                <th className="text-left py-3 px-4 text-sm font-medium text-primary-600">Micro ID</th>
                <th className="text-left py-3 px-4 text-sm font-medium text-primary-600">Ubicación</th>
                <th className="text-left py-3 px-4 text-sm font-medium text-primary-600">Nivel de Ruido</th>
                <th className="text-left py-3 px-4 text-sm font-medium text-primary-600">Estado</th>
                <th className="text-left py-3 px-4 text-sm font-medium text-primary-600">Última Actualización</th>
              </tr>
            </thead>
            <tbody>
              {sensorData.map((sensor) => (
                <tr key={sensor.micro_id} className="border-b border-primary-100 hover:bg-primary-50">
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center space-x-2">
                      <div className={`w-3 h-3 rounded-full ${
                        sensor.value >= 85 ? 'bg-red-500' :
                        sensor.value >= 70 ? 'bg-orange-500' :
                        sensor.value >= 50 ? 'bg-yellow-500' : 'bg-green-500'
                      }`}></div>
                  ...
                  <td className="py-3 px-4">
                    <span className={`px-2 py-1 text-xs font-medium rounded-lg ${
                      sensor.value >= 85 ? 'text-red-600 bg-red-50 border border-red-200' :
                      sensor.value >= 70 ? 'text-orange-600 bg-orange-50 border border-orange-200' :
                      sensor.value >= 50 ? 'text-yellow-600 bg-yellow-50 border border-yellow-200' :

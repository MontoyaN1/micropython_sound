...
import { Calendar, BarChart3, Download, Filter, Clock, CalendarDays } from 'lucide-react'
import { fetchRecentData, fetchSensorStatistics, fetchDateRangeData } from '../services/api'
...
const HistoricalPage = () => {
  ...
  const loadHistoricalData = async () => {
    ...
    try {
      if (dateMode === 'range') {
        ...
        const hoursToFetch = selectedDate.toDateString() === now.toDateString() 
          ? 24 
          : Math.min(diffHours, 720) // Máximo 30 días (720 horas)
        
        console.log(`Consultando ${hoursToFetch} horas desde ${selectedDate.toLocaleDateString()}`)
  ...
  const handleExportData = () => {
    const filteredData = historicalData.filter(row => row.micro_id === selectedSensor)
    ...
    const csvContent = [
      ['Timestamp', 'Micro ID', 'Ubicación', 'Nivel de Ruido (dB)', 'Latitud', 'Longitud'],
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-primary-900">Datos Históricos</h1>
       ...
       <div className="card">
         <h3 className="font-semibold text-lg mb-4">Estadísticas del Sensor {selectedSensor}</h3>
         {sensorStats ? (
           <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
             <div className="bg-primary-50 rounded-xl p-4">
               <div className="text-sm text-primary-600 mb-1">Promedio</div>
               <div className="text-2xl font-bold">{sensorStats.mean.toFixed(1)} dB</div>
             </div>
             <div className="bg-primary-50 rounded-xl p-4">
               <div className="text-sm text-primary-600 mb-1">Máximo</div>
               <div className="text-2xl font-bold">{sensorStats.max.toFixed(1)} dB</div>
             </div>
             <div className="bg-primary-50 rounded-xl p-4">
               <div className="text-sm text-primary-600 mb-1">Mínimo</div>
               <div className="text-2xl font-bold">{sensorStats.min.toFixed(1)} dB</div>
             </div>
             <div className="bg-primary-50 rounded-xl p-4">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-primary-200">
                  <th className="text-left py-3 px-4 text-sm font-medium text-primary-600">Fecha y Hora</th>
                  ...
                  <th className="text-left py-3 px-4 text-sm font-medium text-primary-600">Nivel de Ruido (dB)</th>
                  <th className="text-left py-3 px-4 text-sm font-medium text-primary-600">Coordenadas</th>
                </tr>
              </thead>
              <tbody>
                 {historicalData
                   ...
                   <tr key={index} className="border-b border-primary-100 hover:bg-primary-50">
                     ...
                     <td className="py-3 px-4">
                       <div className={`font-bold ${
                         row.value >= 85 ? 'text-red-600' :
                         row.value >= 70 ? 'text-orange-600' :
                         row.value >= 50 ? 'text-yellow-600' : 'text-green-600'
                       }`}>
             ...
             {historicalData.filter(row => row.micro_id === selectedSensor).length > 50 && (
               <div className="text-center py-4 text-primary-600 text-sm">
                 Mostrando 50 de {historicalData.filter(row => row.micro_id === selectedSensor).length} registros del sensor {selectedSensor}.
               </div>
             )}
          </div>
        )}

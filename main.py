...
st.set_page_config(
    page_title="Mapa de Contaminación Acústica",
    page_icon="🔊",
    layout="wide"
)
...
def generar_datos_sensores(num_sensores=6):
    ...
    for i in range(num_sensores):
        ...
        sensores.append({'id': f'Sensor {i+1}', 'x': x, 'y': y, 'ruido': max(40, min(ruido, 100))})
...
with st.sidebar:
    ...
    tipo_visualizacion = st.radio(
        ...
        ["Mapa de Calor", "Puntos de Sensor", "Contornos"]
    )
...
with col1:
    ...
    ax.set_xlim(0, 10)
    ax.set_ylim(0, 10)
    ax.add_patch(plt.Rectangle((0, 0), 10, 10, fill=False, edgecolor='black', linewidth=2))
    ...
    if tipo_visualizacion == "Mapa de Calor":
        ...
        heatmap = ax.imshow(zi, extent=[0, 10, 0, 10], origin='lower', 
                           cmap=esquema_color, alpha=opacidad, aspect='auto')
    ...
    ax.scatter(x[max_ruido_idx], y[max_ruido_idx], s=300, facecolors='none', 
              edgecolors='red', linewidths=2, label='Epicentro')
...
with col2:
    ...
    st.dataframe(sensores.style.highlight_max(axis=0, color='#ffcc80'), 
    ...
    ax2.bar(sensores['id'], sensores['ruido'], color=plt.cm.viridis(sensores['ruido']/100))
...
# Pie de página
st.markdown("---")
st.markdown("Sistema de monitoreo de contaminación acústica | Desarrollado con Streamlit y MicroPython")